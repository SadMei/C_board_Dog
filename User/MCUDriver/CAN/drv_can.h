...
//	void MITControl();
	...
	drv_canC(uint8_t Motor_ID, uint8_t Motor_Type, CAN_HandleTypeDef* Motor_CanLine,float Motor_AngleUpLimit,float Motor_AngleLowerLimit,int8_t signer,float stand_angle,float lay_angle):
		Motor_ID(Motor_ID), Motor_Type(Motor_Type), Motor_CanLine(Motor_CanLine),Motor_AngleLowerLimit(Motor_AngleLowerLimit),Motor_AngleUpLimit(Motor_AngleUpLimit),signer(signer),stand_angle(stand_angle),lay_angle(lay_angle){}; //��ʼ���б�
 private:
	uint8_t send_data[8];
